...
function buildContext(searchResults, hotspots) {
  ...
  // Sort by score descending, with a complexity boost for hotspot files.
  // Each complexity point adds 0.02 to the effective score (capped at +0.30).
  // This ensures high-complexity files rank above equal-scored simpler files.
  const hotspotMap = new Map(hotspots.map(h => [h.file, parseInt(h.complexity) || 0]));
  const complexityBoost = (file) => Math.min(0.30, (hotspotMap.get(file) || 0) * 0.02);

  const topFiles = [...seen.values()]
    .sort((a, b) => (b.score + complexityBoost(b.file)) - (a.score + complexityBoost(a.file)))
    .slice(0, 4);
...
async function runAgentLoop(agent, context) {
  ...
  const attempt = async () => {
    response = await chat(messages, false, agent.temperature ?? 0.1);   // no tools — analysis only
    apiCalls++;
  };
